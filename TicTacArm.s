...
@@@@@@@@@@@@@@@@@@@@@@ Constants for assembler and mmap @@@@@@@@@@@@@@@@@@@@
...
@ Others parameters
		...
		.equ	DELAY,	50000000		@ 5*10^7 empiric value from playtesting, reduce by 100x on bare metal
		.equ	BLINK,	200000000		@ 2*10^8 same here: reduce by 100x on bare metal
...
@@@@@@@@@@@@@@@@@@@@@@@ MEMORY ADDRESS MAPPING @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
		...
		stmfd	sp!, {r4, r5, r6, r7, r8, r9, r10, r11, r12, lr}	@ push all registers we are going to use
        add     fp, sp, #36      @ set our frame pointer
...
@		b	debug						@ jump: to place where log is needed
@resume:								@ return point: to place after log jump

	
@debug:
@		ldr     	r0, =message1     	@ load message string
@		mov     	r1, r6 				@ change to the registers you need to log
@		mov     	r2, r7	
@		mov     	r3, r12
@		bl		printf
@		b		resume
...
@ ATM this is NEVER reached as the program has NO end condition: use it if you implement it
